...
dir = r'C:\Users\ovj\Desktop\test' ### Only works for one file in directory
...
#fname=os.listdir(dir)[2]# Choose file from directory
#filepath=os.path.join(dir, fname)                    

filepath=iovj.file2analyse(dir, 'Batch', '_out') # (directory to search in, 'what to search for', 'what to exclude')
...
for m, IndFit in enumerate(DataIndFit):
    ...
    #            print("hallo")
        if i == 0:        
            ...
            PolData.append(re.search("Polin(.{0,2}\d{0,3})",line).group(1))
    ...
    OutData=OutData.reshape(int(len(OutData)/8), 8)
    header = OutData[0:2,:]

PolNumbers = np.asarray(PolData,dtype=np.float32)

#%%  Save



outpath=os.path.join(dir, re.split('.txt', os.path.basename(filepath))[0]+'_out.txt') 
...
ax1.plot(PolNumbers, OutData[2:,6], 'b-',label='Intensity')
...
'''Plot full Range'''
ax3.plot(np.asarray(PolData, dtype=np.float32), cos2(np.asarray(PolData, dtype=np.float32), *popt),'g-', label='cos^2 fit: theta0=%5.3f' % (popt[0]%90))  # modulo 90...
ax3.plot(np.arange(min(PolNumbers), max(PolNumbers)), cos2(np.arange(min(PolNumbers), max(PolNumbers)), *popt),'r-', label='cos^2 fit smooth: theta0=%5.3f' % (popt[0]%90))  # modulo 90...
