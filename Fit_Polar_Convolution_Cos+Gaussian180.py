...
dir = r'C:\Users\ovj\Desktop\ToDo\180611_9AGNRonAu788_Vac_Cos4_S382\in' ### Only works for one file in directory
...
xdata=data[:-1,0]*(2*np.pi/360)  # this assumes data from 0-(360+1step), and then deletes the last two.
ydata=data[:-1,1] # this assumes data from 0-(360+1step), and then deletes the last two.
errydata=data[:-1,2] # this assumes data from 0-(360+1step), and then deletes the last two.
...
ax1.set_ylabel("Intensity (arb.u.)")
'''Plot full Range'''
plt.plot(theta, intens4I0(theta, *popt),'r-', label='cos^4 fit: theta0=%5.3f, sigma=%5.3f, A=%5.3f' % param)
'''Plot Meas-Data XY-Coordinates (Extended X-Range)'''
plt.plot(nxdata,nydata, 'bx',label='data')
#Add Legend
plt.legend()
...
ax2.set_xlabel("Theta (°)")
ax2.set_ylabel("Intensity (arb.u.)")
ax2.plot(theta, intens4I0(theta, *popt),'r-', label='cos^4 fit: theta0=%5.3f, sigma=%5.3f, A=%5.3f' % param)
ax2.set_xlim([-np.pi,np.pi])
